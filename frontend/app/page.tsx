...
function SupportingSections({ plan }: { plan: TripPlan }) {
  const blocks = [["Flights", "✈", plan.flights, "flight"], ["Where you’ll stay", "⌂", plan.stays, "stay"], ["What the weather might feel like", "☼", plan.weather, "weather"]];
...
export default function Home() {
  const [message, setMessage] = useState(""); const [feedback, setFeedback] = useState(""); const [threadId, setThreadId] = useState<string | null>(null); const [plan, setPlan] = useState<TravelResponse | null>(null); const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const [copied, setCopied] = useState(false);
  ...
  async function submitTravel(event?: FormEvent) { event?.preventDefault(); if (!message.trim() || loading) return; setLoading(true); setError(""); try { const response = await fetch("/api/travel", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: message.trim(), thread_id: threadId }) }); const data = await readResponse(response); if (!response.ok || !data.success) throw new Error(data.error || "TripMate could not create this plan."); setPlan(data); setThreadId(data.thread_id); } catch (err) { setError(err instanceof Error ? err.message : "Something went wrong while planning your trip."); } finally { setLoading(false); } }
  async function submitReview(approved: boolean) { if (!threadId || loading || (!approved && !feedback.trim())) return; setLoading(true); setError(""); try { const response = await fetch("/api/travel/approve", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ thread_id: threadId, approved, feedback: feedback.trim() }) }); const data = await readResponse(response); if (!response.ok || !data.success) throw new Error(data.error || "TripMate could not update this plan."); setPlan(data); setFeedback(""); } catch (err) { setError(err instanceof Error ? err.message : "Something went wrong while updating your trip."); } finally { setLoading(false); } }
